...
/**
 * Площадка, которую читает не бот, а программа на компьютере владельца.
 * Живьём такую площадку из меню не проверить: с сервера бот увидит не список, а
 * проверку браузера, и человек решил бы, что площадка сломана. Поэтому вместо
 * находки честно рассказываем, как это работает, — и сразу даём добавить.
 */
export function catalogLocalOnly(entry: CatalogEntry): { text: string; keyboard: InlineKeyboard } {
  const lines = [
    ...catalogHead(entry),
    "",
    "💻 Эту площадку я читаю с вашего компьютера.",
    "",
    "Она открывается только тем, кто заходит на неё из дома или из офиса, — меня она " +
      "к себе не пускает. Поэтому страницу забирает маленькая программа на вашем " +
      "компьютере и присылает мне, а дальше всё как обычно: я отбираю тендеры по " +
      "вашим словам и присылаю новые.",
  ];
  if (entry.note) lines.push("", `ℹ️ ${escapeHtml(entry.note)}`);
  lines.push("", "Добавляем эту площадку?");
...
/**
 * Что сказать сразу после добавления такой площадки. Читать её прямо сейчас
 * бот не может, поэтому вместо привычного отчёта о находке — что произойдёт
 * дальше и чего ждать. Молчание здесь выглядело бы как поломка.
 */
export function localSiteAddedText(title: string, forOwner = false): string {
  const lines = [
    `✅ Площадка «${escapeHtml(title)}» добавлена.`,
    "",
    "💻 Читать её я буду с вашего компьютера: сама она пускает только тех, кто " +
      "заходит из дома или из офиса.",
    "",
    "Чтобы это заработало, на компьютер нужно один раз поставить маленькую программу — " +
      "она открывает страницу и присылает её мне.",
  ];

  // Владельцу некуда идти за командами: он и есть тот, кто их выполняет.
  // Отправлять его «ждать сообщения от владельца» — тупик ровно там, где
  // инструкция нужнее всего.
  if (forOwner) {
    lines.push(
      "",
      "Откройте на своём компьютере программу «Терминал» и вставьте эти две строки — " +
        "по очереди, каждую с новой строки:",
      "",
      "<code>cd ~/tender_bot</code>",
      "<code>bash scripts/install-collector.sh</code>",
      "",
      "Программа сама скажет, чего ей не хватает, и что делать дальше.",
    );
  } else {
    lines.push(
      "",
      "Готовые команды пришлёт тот, кто настраивал бота: их нужно будет скопировать и вставить.",
    );
  }

  lines.push(
    "",
    "Пока программы нет, тендеры отсюда приходить не будут. Остальные площадки работают как обычно.",
  );
  return lines.join("\n");
}

/** Как в карточке сайта подписан способ обхода. Без единого слова из техники. */
export const VIA_LOCAL_LINE = "Обход: с вашего компьютера";

/**
 * Что сказать после переключения способа обхода. Само по себе изменение в
 * карточке — одна строка, и человек не поймёт, что теперь делать: в одну
 * сторону нужна программа на компьютере, в другую — она больше не нужна.
 */
export function viaLocalSwitchedText(title: string, viaLocal: boolean): string {
  const name = escapeHtml(title);
  if (viaLocal) {
    return [
      `💻 Теперь «${name}» читает ваш компьютер.`,
      "",
      "Сам я на эту площадку больше не хожу — страницу присылает маленькая программа " +
        "с вашего компьютера. Если она ещё не установлена, откройте «Терминал» и вставьте " +
        "эти две строки:",
      "",
      "<code>cd ~/tender_bot</code>",
      "<code>bash scripts/install-collector.sh</code>",
  ...
  return [
    `🌍 Теперь «${name}» читаю я сам.`,
    "",
    "Программа на вашем компьютере для неё больше не нужна: площадка вернётся в обычную " +
      "проверку по расписанию. Если площадка меня к себе не пускает, в карточке появится " +
      "ошибка — тогда верните обход на компьютер той же кнопкой.",
  ].join("\n");
}

/**
 * Приписка к отчёту о проверке, когда среди площадок есть читаемые с
 * компьютера владельца. Без неё кнопка «Проверить сейчас» выглядела бы так,
 * будто она обошла и их тоже — и ничего не нашла.
 */
export function localCheckNote(localSites: number): string | null {
  if (localSites === 0) return null;
  const word = localSites === 1 ? "площадку" : "площадки";
  return (
    `💻 Ещё ${localSites} ${word} читает ваш компьютер — они проверяются сами, ` +
    "по своему расписанию, и в этот список не входят."
  );
}

/** Ответ на «🧪 Проверить» для площадки, которую бот сам открыть не может. */
export const VIA_LOCAL_TEST_TEXT = [
  "💻 Эту площадку я не открываю сам — её присылает программа с вашего компьютера.",
  "",
  "Проверить её отсюда не выйдет: мне она список не показывает.",
  "В карточке площадки видно время последнего сбора — строка «Проверен»; " +
    "если что-то пошло не так, там же будет причина.",
].join("\n");

/**
 * Через сколько часов молчания домашнего сборщика пора сказать об этом вслух.
 * Обычный сбор идёт каждые 15 минут, так что три часа — это уже не «компьютер
 * ненадолго уснул», а «что-то сломалось».
 */
export const LOCAL_STALE_HOURS = 3;

/** «1 час», «2 часа», «5 часов» — иначе бот пишет как машина. */
...
/**
 * Строка про молчащий домашний сбор. Без неё карточка площадки, которую
 * компьютер не присылает уже неделю, выглядит совершенно здоровой: дата
 * последней удачной проверки на месте, ошибки нет — а тендеры не приходят.
 */
export function localStaleLine(site: Site, now: number = Date.now()): string | null {
  if (!site.via_local) return null;

  if (!site.last_checked_at) {
    return (
      "⚠️ С вашего компьютера сюда ещё ничего не приходило — похоже, программа сбора " +
      "ещё не установлена."
    );
  }

  const checkedAt = Date.parse(site.last_checked_at);
  if (!Number.isFinite(checkedAt)) return null;

  const hours = Math.floor((now - checkedAt) / 3_600_000);
  if (hours < LOCAL_STALE_HOURS) return null;

  return (
    `⚠️ Последний раз страницу присылали ${agoText(hours)}, а обычно это происходит ` +
    "каждые 15 минут. Проверьте, включён ли компьютер и есть ли на нём интернет."
  );
}
